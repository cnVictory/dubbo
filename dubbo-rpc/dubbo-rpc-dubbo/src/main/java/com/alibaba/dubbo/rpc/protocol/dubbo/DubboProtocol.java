...
public class DubboProtocol extends AbstractProtocol {
    ...
    Invoker<?> getInvoker(Channel channel, Invocation inv) throws RemotingException {
        ...
        // 获取 Invoker 对象，并返回
        // 获取与指定服务对应的 Invoker 实例，并通过 Invoker 的 invoke 方法调用服务逻辑。invoke 方法定义在 AbstractProxyInvoker 中
    ...
    public <T> Exporter<T> export(Invoker<T> invoker) throws RpcException {
        ...
        // export service.
        // 组装servicekey  key=xx.xxx.xxx.XXService:20880   服务名+端口号
        String key = serviceKey(url);

        // 创建 DubboExporter   invoker作为dubboExporter的属性注入
    ...
    // 第二是创建服务器实例。第三是检测是否支持 client 参数所表示的
    // Transporter 拓展，不存在也是抛出异常。两次检测操作所对应的代码比较直白了，无需多说
    private ExchangeServer createServer(URL url) {
        // send readonly event when server closes, it's enabled by default
        url = url.addParameterIfAbsent(Constants.CHANNEL_READONLYEVENT_SENT_KEY, Boolean.TRUE.toString());
        // enable heartbeat by default
        // 添加心跳检测配置到 url 中  默认1分钟
    ...
    public <T> Invoker<T> refer(Class<T> serviceType, URL url) throws RpcException {
        optimizeSerialization(url);
        // create rpc invoker.  创建一个Reference的rpc client
